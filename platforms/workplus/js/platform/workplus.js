...
(function(window) {
    ...
    */
    app.load = function(option) {
        ...
        try {
            ...
            mue.showLog("param 参数值必须是一个{}对象", "mue.load:web")
            return;
        }
        ...
        // 生成一个新的url地址
        newurl = mue.setUrlParams(params.url, params.param);
    ...
    */
    var globalVariable = mue.storage({ local: false });
    ...
    */
    ...
    @param option.fail {function}  下载失败的回调
    		...
    		app.download({
                ...
                url:"http://www.muejs.com/demo/images/applogo.png",
                name:"考试大纲4.png",
                success: function (res) {
                  app.openFile(res);
                },
	...
	请求返回数据：
  {
	...
	*/
    app.upload = function(option) {
            ...
            options.fileName = option.name || fileURL.substr(fileURL.lastIndexOf('/') + 1) || mue.guid() + type;
